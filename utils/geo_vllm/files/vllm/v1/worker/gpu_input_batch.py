...
class InputBatch:
    ...
    def __init__(
        ...
        self.mix_lambda = torch.zeros((max_num_reqs, ),
                                      dtype=torch.float32,
                                      device=device)
        self.mix_lambda_cpu_tensor = torch.zeros((max_num_reqs, ),
                                                 dtype=torch.float32,
                                                 device="cpu",
                                                 pin_memory=pin_memory)
        self.mix_lambda_cpu = self.mix_lambda_cpu_tensor.numpy()
    ...
    def add_request(
        ...
        if sampling_params := request.sampling_params:
            ...
            self.mix_lambda_cpu[req_index] = sampling_params.geo_lambda
            ...
            self.mix_lambda_cpu[req_index] = 0.0
    ...
    def condense(self) -> None:
        ...
        while empty_req_indices:
            ...
            self.mix_lambda_cpu[empty_index] = self.mix_lambda_cpu[
                last_req_index]
            self.num_accepted_tokens_cpu[
    ...
    def _make_sampling_metadata(self) -> SamplingMetadata:
        ...
        copy_slice(self.mix_lambda_cpu_tensor, self.mix_lambda, num_reqs)
        mix_lambda_tensor = None
        if num_reqs > 0:
            mix_lambda_tensor = self.mix_lambda[:num_reqs]
