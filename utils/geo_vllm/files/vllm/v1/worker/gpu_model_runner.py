...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def execute_model(
        ...
        with record_function_or_nullcontext("Postprocess"):
            logits_base_tensor: Optional[torch.Tensor] = None
            logits_lora_tensor: Optional[torch.Tensor] = None

            if self.use_aux_hidden_state_outputs:
                # True when EAGLE 3 is used.
                hidden_state_payload, aux_hidden_states = model_output
            else:
                # Common case.
                hidden_state_payload = model_output
                aux_hidden_states = None

            hidden_states = hidden_state_payload
            hidden_states_lora: Optional[torch.Tensor] = None

            if not self.broadcast_pp_output:
                # Common case.
                if not get_pp_group().is_last_rank:
                    # Return the intermediate tensors.
                    assert isinstance(hidden_state_payload, IntermediateTensors)
                    hidden_state_payload.kv_connector_output = kv_connector_output
                    return hidden_state_payload

                if self.is_pooling_model:
                    # Return the pooling output.
                    assert not isinstance(hidden_state_payload, tuple)
                    output = self._pool(hidden_state_payload,
                                        num_scheduled_tokens,
                                        num_scheduled_tokens_np)
                    output.kv_connector_output = kv_connector_output
                    return output

                if isinstance(hidden_state_payload, tuple):
                    hidden_states, hidden_states_lora = hidden_state_payload
                else:
                    hidden_states = hidden_state_payload
                    hidden_states_lora = None

                sample_hidden_states_base = hidden_states[logits_indices]
                if hidden_states_lora is not None:
                    sample_hidden_states_for_logits = (
                        sample_hidden_states_base,
                        hidden_states_lora[logits_indices],
                    )
                else:
                    sample_hidden_states_for_logits = sample_hidden_states_base

                logits_result = self.model.compute_logits(
                    sample_hidden_states_for_logits)
                if isinstance(logits_result, tuple):
                    logits_base_tensor, logits_lora_tensor = logits_result
                else:
                    logits_base_tensor = logits_result
                    logits_lora_tensor = None
                logits = logits_base_tensor
                sample_hidden_states = sample_hidden_states_base
            else:
                # Rare case.
                ...
                if not get_pp_group().is_last_rank:
                    ...
                    if isinstance(logits, tuple):
                        logits_base_tensor, logits_lora_tensor = logits
                        logits = logits_base_tensor
                    else:
                        logits_base_tensor = logits
                        logits_lora_tensor = None
            ...
            if (logits_lora_tensor is not None
                ...
                mix_lambda = sampling_metadata.mix_lambda
                if mix_lambda.shape[0] != logits_base_tensor.shape[0]:
                    mix_lambda = mix_lambda[:logits_base_tensor.shape[0]]
                mix_lambda = mix_lambda.to(torch.float32).unsqueeze(-1)
                base_log_probs = torch.log_softmax(logits_base_tensor,
                                                   dim=-1,
                                                   dtype=torch.float32)
                ...
                logits = logits_base_tensor if logits_base_tensor is not None else logits
    ...
    def _dummy_run(
        ...
        if isinstance(hidden_states, tuple):
            selected_hidden_states = tuple(
                state[logit_indices] for state in hidden_states)
        else:
            selected_hidden_states = hidden_states[logit_indices]

        return hidden_states, selected_hidden_states
    ...
    def _dummy_sampler_run(
        ...
        if isinstance(hidden_states, tuple):
            hidden_states = tuple(torch.rand_like(state)
        ...
        if isinstance(logits, tuple):
            logits_for_sampling = logits[0]
        else:
            logits_for_sampling = logits

        num_reqs = logits_for_sampling.size(0)
        ...
        dummy_metadata = SamplingMetadata(
            ...
            top_k=dummy_tensors(logits_for_sampling.size(1) - 1),
            generators={},
        ...
        try:
            sampler_output = self.sampler(logits=logits_for_sampling,
        ...
        if self.speculative_config:
            ...
            target_logits = torch.randn(num_tokens,
                                        ...
                                        device=self.device,
                                        dtype=logits_for_sampling.dtype)
