...
class Scheduler(SchedulerInterface):
    ...
    def update_from_output(
        ...
        base_token_logps = model_runner_output.base_token_logps
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            stopped = False
            new_logprobs = None
            req_base_token_logps = None
            new_token_ids = generated_token_ids
            kv_transfer_params = None
            status_before_stop = request.status

            if base_token_logps:
                base_seq = base_token_logps[req_index]
                if base_seq:
                    req_base_token_logps = list(base_seq)

            # Check for stop and update request status.
            if new_token_ids:
                new_token_ids, stopped = self._update_request_with_output(
                    request, new_token_ids)
                if req_base_token_logps is not None:
                    req_base_token_logps = req_base_token_logps[:len(
                        new_token_ids)]
            ...
            if new_token_ids or pooler_output is not None \
                ...
                outputs[request.client_index].append(
                    EngineCoreOutput(
                        ...
                        base_token_logps=req_base_token_logps,
                    ))
            else:
