...
class CompletionOutput:
    ...
    Args:
        ...
        base_token_logps: Optional per-token base-policy log probabilities
            aligned with `token_ids`. Used by custom trainers that rely on
            mixed-behavior rollouts.
    """
    ...
    base_token_logps: Optional[list[float]] = None
            ...
            for i, completion in enumerate(self.outputs):
                if completion.index == next_completion.index:
                    if aggregate:
                        ...
                        if next_completion.base_token_logps:
                            if completion.base_token_logps is None:
                                completion.base_token_logps = list(
                                    next_completion.base_token_logps)
                            else:
                                completion.base_token_logps.extend(
                                    next_completion.base_token_logps)
