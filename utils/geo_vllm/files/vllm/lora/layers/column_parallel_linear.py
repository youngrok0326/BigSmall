...
def _mcp_apply(
    ...
    *,
    ...
    if lora_x is None and not return_separate:
        output = layer.base_layer.quant_method.apply(layer.base_layer, base_x,
                                                     bias)
        base_x = base_x.view(-1, base_x.shape[-1])
        output, orig_shape = output.view(-1, output.shape[-1]), output.shape

        buffers = torch.zeros(
            (layer.n_slices, base_x.shape[0], layer.lora_a_stacked[0].shape[2]),
            dtype=torch.float32,
            device=base_x.device,
        )
        shrunk = layer.punica_wrapper.add_shrink(buffers, base_x,
                                                 layer.lora_a_stacked, 1.0)
        if not current_platform.can_update_inplace():
            buffers = cast(torch.Tensor, shrunk)

        buffers = tensor_model_parallel_all_gather(buffers)

        updated = layer.punica_wrapper.add_expand(
            output,
            buffers,
    ...
    lora_x = base_x if lora_x is None else lora_x

    base_out = layer.base_layer.quant_method.apply(layer.base_layer, base_x,
                                                   bias)
    base_shape = base_out.shape
    base_flat = base_out.view(-1, base_out.shape[-1])

    if lora_x is base_x:
        lora_flat = base_flat.clone()
        lora_shape = base_shape
    else:
        lora_out = layer.base_layer.quant_method.apply(layer.base_layer,
                                                       lora_x, bias)
        lora_shape = lora_out.shape
        lora_flat = lora_out.view(-1, lora_out.shape[-1])

    lora_input_flat = (base_x.view(-1, base_x.shape[-1]) if lora_x is base_x else
                       lora_x.view(-1, lora_x.shape[-1]))

    buffers = torch.zeros(
        (layer.n_slices, lora_input_flat.shape[0],
         layer.lora_a_stacked[0].shape[2]),
        dtype=torch.float32,
        device=lora_input_flat.device,
    )
    shrunk = layer.punica_wrapper.add_shrink(buffers, lora_input_flat,
                                             layer.lora_a_stacked, 1.0)
    if not current_platform.can_update_inplace():
        buffers = cast(torch.Tensor, shrunk)
    ...
    if not current_platform.can_update_inplace():
        lora_flat = cast(torch.Tensor, updated)

    base_out = base_flat.view(*base_shape)
    lora_out = lora_flat.view(*lora_shape)

    return (base_out, lora_out) if return_separate else lora_out
    ...
    def forward(
        ...
        *,
        ...
        if self.base_layer.gather_output and self.tp_size > 1:
            ...
            lora_output = (tensor_model_parallel_all_gather(
