...
class InductorAdaptor(CompilerInterface):
    ...
    def compile(
        ...
        def _get_callable_code(obj: Any):
            """
            Retrieve a code object from a callable, handling wrappers
            like functools.partial or bound methods that do not expose
            __code__ directly.
            """
            visited: set[int] = set()
            current = obj
            while current is not None and id(current) not in visited:
                ...
                if func is not None and func is not current:
                    current = func
                    continue
                break
            return None
        ...
        if torch.__version__.startswith("2.5"):
            ...
            def hijack_load(*args, **kwargs):
                ...
                code_obj = _get_callable_code(compiled_fn)
                file_path = code_obj.co_filename if code_obj else None
                if (file_path is None
                        or not file_path.startswith(self.base_cache_dir)):
                    # hooked in the align_inputs_from_check_idxs function
                    # in torch/_inductor/utils.py
                    closure = getattr(compiled_fn, "__closure__", None) or ()
                    for cell in closure:
                        if not callable(cell.cell_contents):
                            continue
                        code = _get_callable_code(cell.cell_contents)
                        if code is None:
                            continue
