...
def grpo_compute_loss(
    ...
    **kwargs
    ...
    geo_lambda = float(kwargs.get("geo_lambda", 1.0))
    base_token_logps = kwargs.get("base_token_logps", None)
    if base_token_logps is not None and not torch.is_tensor(base_token_logps):
        base_token_logps = torch.tensor(base_token_logps)
    ...
    if old_logits is not None: 
        ...
        log_behavior = new.detach()

    if base_token_logps is not None and geo_lambda < 1.0:
        base_token_logps = base_token_logps.to(log_behavior.device, dtype=log_behavior.dtype)
        log_behavior = geo_lambda * log_behavior + (1.0 - geo_lambda) * base_token_logps
    
    log_ratio = new - log_behavior
...
class UnslothEfficientGRPO(torch.autograd.Function):
    ...
    def forward(ctx, _new_hidden_states, _old_hidden_states, _ref_hidden_states, lm_head, _input_ids, _mask, _advantages, beta, scaler = None, n_chunks = 1, extra_kwargs=None):
        if extra_kwargs is None:
            extra_kwargs = {}
        def compute_loss(new_hidden_states, old_hidden_states, ref_hidden_states, input_ids, mask, advantages, scaling, extra_kwargs):
            chunk_kwargs = extra_kwargs or {}
        ...
        def accumulate_chunk(
            ...
            (chunk_grad_input,), (chunk_loss, (unscaled_loss, chunk_completion_length, chunk_mean_kl,)) = torch.func.grad_and_value(
                ...
                argnums = (0,),
                has_aux = True,
            )(new_hidden_states_j, old_hidden_states_j, ref_hidden_states_j, input_ids_j, mask_j, advantages_j, scaling, extra_kwargs_j)
        ...
        template_kwargs = dict(extra_kwargs)
        base_token_logps = template_kwargs.pop("base_token_logps", None)
        geo_lambda = float(template_kwargs.get("geo_lambda", 1.0))
        template_kwargs["geo_lambda"] = geo_lambda

        if base_token_logps is not None:
            base_token_logps_chunks = torch.chunk(base_token_logps, chunks = n_chunks, dim = 0)
        else:
            base_token_logps_chunks = [None] * n_chunks

        chunk_extra_kwargs = []
        for idx in range(n_chunks):
            chunk_kwargs = dict(template_kwargs)
            base_chunk = base_token_logps_chunks[idx]
            if base_chunk is not None:
                chunk_kwargs["base_token_logps"] = base_chunk
            chunk_extra_kwargs.append(chunk_kwargs)
        ...
        for (grad_inputs_j, new_hidden_states_j, old_hidden_states_j, ref_hidden_states_j,  input_ids_j, mask_j, advantages_j, extra_kwargs_j,) in \
            zip(grad_inputs_chunks, new_hidden_states, old_hidden_states, ref_hidden_states, input_ids, mask, advantages, chunk_extra_kwargs):
...
def grpo_accumulated_loss(
    ...
    **kwargs,
    ...
    base_token_logps = extra_kwargs.get("base_token_logps", None)
    geo_lambda = float(extra_kwargs.get("geo_lambda", getattr(trainer, "geo_lambda", 1.0)))
    extra_kwargs["geo_lambda"] = geo_lambda
