...
def grpo_trainer__generate_and_score_completions(function_name, function):
    ...
    pattern = (
        ...
        r"        # Decode the generated completions"
    )
    ...
    function = re.sub(pattern, replacement, function, flags=re.DOTALL)
...
def grpo_trainer_compute_loss(function_name, function):
    ...
    def compute_loss(self, model, inputs, return_outputs = False, num_items_in_batch = None):
        ...
        ref_hidden_states = inputs.get("sampling_per_token_logps", None)
        # x - x.detach() allows for preserving gradients from x
        advantages = inputs["advantages"]
        base_token_logps = inputs.get("base_token_logps")
        geo_lambda = getattr(self, "geo_lambda", 1.0)
