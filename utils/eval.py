...
def cleanup_gpu():
    """Force clean GPU memory between model loads"""
...
def test_model(cfg: DictConfig, lora_name: str, merged_directory: str, results: dict):
    ...
    # Force cleanup GPU before loading model
    cleanup_gpu()
    ...
    try:
        if lora_name == "":
            ...
            model, tokenizer = FastLanguageModel.from_pretrained(
                ...
                tensor_parallel_size=1,       # Ensure single GPU
                disable_custom_all_reduce=True, # May help with VLLM issues
            )
        else:
            import subprocess
        ...
        # Force cleanup GPU after testing
        del model, tokenizer
        cleanup_gpu()
        ...
        print(f"Error testing {lora_name}: {e}")
        # Force cleanup in case of error
        cleanup_gpu()
        return results or {"error": str(e)}
