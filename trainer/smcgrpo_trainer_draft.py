...
class _SMCVariableTrajectoryKind(str, Enum):
    LEGACY = "legacy"
    DROPPED = "dropped"


@dataclass(slots=True)
class _SMCVariableMeta:
    group_sizes: list[int]
    group_counts: list[int]
    group_indices: list[int]
    kinds: list[_SMCVariableTrajectoryKind]
    drop_steps: list[int]
...
def _smc_variable_is_bundle(payload: Any) -> bool:
    return isinstance(payload, dict) and "group_sizes" in payload and "completions" in payload


def _smc_variable_flatten_bundle(
    payload: dict[str, Any],
    fallback_total_steps: int,
) -> tuple[list[list[int]], _SMCVariableMeta]:
    completions_by_group: list[list[list[int]]] = payload.get("completions", [])  # type: ignore[assignment]
    group_sizes: list[int] = payload.get("group_sizes", [])  # type: ignore[assignment]
    ...
    if len(group_sizes) != len(completions_by_group):
        raise ValueError("SMC bundle is malformed: group_sizes length mismatch")
    ...
    for group_idx, primary in enumerate(completions_by_group):
        extras: Iterable[tuple[int, list[int]]] = [] if group_idx >= len(saved) else saved[group_idx]
        extras_list = list(extras)
        total_step_estimate = (
            int(reasoning_steps[group_idx])
            if group_idx < len(reasoning_steps)
            else max(fallback_total_steps, max((step + 1 for step, _ in extras_list), default=fallback_total_steps))
        )

        for seq in primary:
            flattened.append(seq)
            group_indices.append(group_idx)
            kinds.append(_SMCVariableTrajectoryKind.LEGACY)
            drop_steps.append(-1)
            total_steps.append(total_step_estimate)

        for step_idx, seq in extras_list:
            flattened.append(seq)
            group_indices.append(group_idx)
            kinds.append(_SMCVariableTrajectoryKind.DROPPED)
            drop_steps.append(int(step_idx))
            total_steps.append(max(total_step_estimate, int(step_idx) + 1))

        group_counts.append(len(primary) + len(extras_list))

    meta = _SMCVariableMeta(
...
def _smc_variable_compute_drop_reward(
    kind_ids: Sequence[str],
    drop_steps: Sequence[int],
    total_steps: Sequence[int],
    device: torch.device,
) -> torch.Tensor:
    drop_mask = torch.tensor([kind == _SMCVariableTrajectoryKind.DROPPED.value for kind in kind_ids], dtype=torch.bool, device=device)
    reward = torch.zeros(len(kind_ids), dtype=torch.float32, device=device)
...
def _smc_variable_generate_and_score(self: "SMCGRPOTrainer", inputs: list[dict[str, Any]]) -> dict[str, Any]:
    if not (self.use_vllm or hasattr(self, "llm")):
        return self._generate_and_score_completions_legacy(inputs)
    ...
    has_images = "image" in inputs[0]
    images_full = [example.get("image") for example in inputs] if has_images else None
    kwargs_for_tokenizer = {}
    if has_images and images_full is not None:
        kwargs_for_tokenizer = {"images": [[img] for img in images_full]}
        for prompt in prompts:
            if isinstance(prompt, list):
                for message in prompt:
                    if not isinstance(message, dict):
    ...
    if not _smc_variable_is_bundle(payload):
        return self._generate_and_score_completions_legacy(inputs)

    completion_ids_list, meta = _smc_variable_flatten_bundle(
        payload,
        getattr(getattr(smc_runner, "sg", None), "max_steps", int(self.max_completion_length)),
    )

    if not meta.group_counts:
        raise ValueError("Received empty SMC bundle; cannot continue training")

    if len(inputs) % self.num_generations != 0:
        raise ValueError("Dataset batch does not align with configured num_generations")

    num_groups_local = len(meta.group_counts)
    group_examples = [copy.deepcopy(inputs[idx * self.num_generations]) for idx in range(num_groups_local)]
    group_prompts = [copy.deepcopy(prompts[idx * self.num_generations]) for idx in range(num_groups_local)]
    group_original_prompts = [copy.deepcopy(original_prompts[idx * self.num_generations]) for idx in range(num_groups_local)]
    group_prompts_text = [prompts_text[idx * self.num_generations] for idx in range(num_groups_local)]
    group_images = [images_full[idx * self.num_generations] for idx in range(num_groups_local)] if has_images and images_full is not None else None
    ...
    for group_idx in meta.group_indices:
        cloned_example = copy.deepcopy(group_examples[group_idx])
        cloned_example["prompt"] = copy.deepcopy(group_prompts[group_idx])
        expanded_inputs.append(cloned_example)
        expanded_prompts.append(copy.deepcopy(group_prompts[group_idx]))
        expanded_original_prompts.append(copy.deepcopy(group_original_prompts[group_idx]))
        expanded_prompts_text.append(group_prompts_text[group_idx])
        if group_images is not None:
            expanded_images.append(group_images[group_idx])

    kwargs_expanded = {}
    if expanded_images:
        kwargs_expanded["images"] = [[img] for img in expanded_images]

    prompt_inputs_expanded = self.processing_class(
        text=expanded_prompts_text,
        return_tensors="pt",
        padding=True,
        padding_side="left",
        add_special_tokens=False,
    ...
    prompt_inputs_expanded = super()._prepare_inputs(prompt_inputs_expanded)
    prompt_ids = prompt_inputs_expanded["input_ids"]
    prompt_mask = prompt_inputs_expanded["attention_mask"]
    ...
    if self.max_prompt_length is not None:
        ...
        expanded_prompts_text = self.processing_class.batch_decode(
            prompt_ids,
            skip_special_tokens=False,
            clean_up_tokenization_spaces=False,
        )
        expanded_prompts_text = [re.sub(rf"^({re.escape(self.pad_token)})+", "", text) for text in expanded_prompts_text]
        if self.image_token is not None:
            expanded_prompts_text = [
                re.sub(rf"({re.escape(self.image_token)})+", self.image_token, text)
                for text in expanded_prompts_text
            ]

    completion_tensors = [torch.tensor(ids, device=device) for ids in completion_ids_list]
    ...
    completion_lengths = completion_mask.sum(1)
    if self.mask_truncated_completions:
        truncated_completions = ~has_eos
        if truncated_completions.any():
            completion_mask = completion_mask * (~truncated_completions).unsqueeze(1).int()
    ...
    if logits_to_keep == 0:
        ...
        if self.args.gradient_accumulation_steps % generate_every != 0:
            old_per_token_logps, _ = self._get_per_token_logps_and_entropies(
                ...
                logits_to_keep,
                batch_size,
                pixel_values=prompt_inputs_expanded.get("pixel_values"),
                image_grid_thw=prompt_inputs_expanded.get("image_grid_thw"),
                pixel_attention_mask=prompt_inputs_expanded.get("pixel_attention_mask"),
                image_sizes=prompt_inputs_expanded.get("image_sizes"),
            )
        else:
            old_per_token_logps = None
    ...
    completion_ids_for_rewards = [
        [token.item() for token, mask in zip(row, mask_row) if mask]
        for row, mask_row in zip(completion_ids, completion_mask)
    ]

    completions_text = self.processing_class.batch_decode(completion_ids, skip_special_tokens=True)
    prompts_for_completions = copy.deepcopy(expanded_prompts)
    if is_conversational(inputs[0]):
        completions: list[Any] = []
        for prompt, completion in zip(prompts_for_completions, completions_text):
            bootstrap = ""
            if isinstance(prompt, list) and prompt and isinstance(prompt[-1], dict) and prompt[-1].get("role") == "assistant":
                bootstrap = prompt.pop()["content"]
            completions.append([{"role": "assistant", "content": bootstrap + completion}])
    else:
        completions = completions_text

    rewards_per_func = self._smc_variable_legacy_calculate_rewards(
        expanded_inputs,
        expanded_original_prompts,
        completions,
        completion_ids_for_rewards,
    )

    local_group_counts = meta.group_counts
    local_group_indices = meta.group_indices
    local_kind_ids = [kind.value for kind in meta.kinds]

    group_counts_across = gather_object(local_group_counts)
    group_offsets_by_rank: list[int] = []
    running_groups = 0
    for counts in group_counts_across:
        group_offsets_by_rank.append(running_groups)
        running_groups += len(counts)
    num_groups_global = running_groups

    group_offset = group_offsets_by_rank[self.accelerator.process_index]
    local_global_group_indices = [group_offset + idx for idx in local_group_indices]

    group_indices_across = gather_object(local_global_group_indices)
    kind_ids_across = gather_object(local_kind_ids)
    drop_steps_across = gather_object(meta.drop_steps)
    total_steps_across = gather_object(meta.total_steps)
    local_sample_count = len(local_global_group_indices)
    sample_counts_across = gather_object(local_sample_count)

    global_group_indices = [idx for sub in group_indices_across for idx in sub]
    global_kind_ids = [kind for sub in kind_ids_across for kind in sub]
    global_drop_steps = [step for sub in drop_steps_across for step in sub]
    global_total_steps = [step for sub in total_steps_across for step in sub]

    if rewards_per_func.size(0) != len(global_group_indices):
        raise ValueError("Mismatch between reward tensor and SMC metadata")

    drop_reward = _smc_variable_compute_drop_reward(
        global_kind_ids,
        global_drop_steps,
        global_total_steps,
        rewards_per_func.device,
    )
    if rewards_per_func.size(1) >= 1:
        rewards_per_func[:, 0] = drop_reward
    ...
    group_means = torch.zeros(num_groups_global, dtype=torch.float32, device=rewards_per_func.device)
    group_stds = torch.zeros_like(group_means)

    for group_idx in range(num_groups_global):
        mask = group_indices_tensor == group_idx
        if mask.any():
            values = weighted_rewards[mask]
            group_means[group_idx] = torch.nanmean(values)
            group_stds[group_idx] = nanstd(values)

    std_zero_mask = torch.isclose(group_stds, torch.zeros_like(group_stds))

    sample_group_means = group_means[group_indices_tensor]
    sample_group_stds = group_stds[group_indices_tensor]
    advantages_all = weighted_rewards - sample_group_means
    if self.scale_rewards:
        advantages_all = advantages_all / (sample_group_stds + 1e-4)

    sample_offsets_by_rank: list[int] = []
    running_samples = 0
    for count in sample_counts_across:
        sample_offsets_by_rank.append(running_samples)
        running_samples += count

    local_sample_offset = sample_offsets_by_rank[self.accelerator.process_index]
    local_sample_count = len(local_global_group_indices)
    sample_slice = slice(local_sample_offset, local_sample_offset + local_sample_count)
    advantages_local = advantages_all[sample_slice].clone()
    ...
    if num_groups_global > 0:
        self._metrics[mode]["reward"].append(group_means.mean().item())
        self._metrics[mode]["reward_std"].append(group_stds.mean().item())
        self._metrics[mode]["frac_reward_zero_std"].append(std_zero_mask.float().mean().item())
    else:
    ...
    self._logs["prompt"].extend(gather_object(expanded_prompts_text))
    self._logs["completion"].extend(gather_object(completions_text))
    for idx, reward_func_name in enumerate(self.reward_func_names):
        self._logs["rewards"][reward_func_name].extend(rewards_per_func[:, idx].detach().cpu().tolist())
    self._logs["advantages"].extend(advantages_all.detach().cpu().tolist())
    if expanded_images:
        self._logs["image"].extend(gather_object(expanded_images))
    ...
    for key in ("pixel_values", "image_grid_thw", "pixel_attention_mask", "image_sizes"):
        if key in prompt_inputs_expanded:
            output[key] = prompt_inputs_expanded[key]

    self._smc_variable_last_meta = meta
    return output


def _smc_variable_enable(self: "SMCGRPOTrainer") -> None:
    if getattr(self, "_smc_variable_enabled", False):
        return
    self._generate_and_score_completions_legacy = self._generate_and_score_completions
    self._smc_variable_legacy_calculate_rewards = self._calculate_rewards
    self._generate_and_score_completions = MethodType(_smc_variable_generate_and_score, self)
    self._smc_variable_enabled = True


SMCGRPOTrainer.enable_variable_smc = _smc_variable_enable


_SMCGRPOTrainer__init__ = SMCGRPOTrainer.__init__


def _smc_variable_trainer_init(self, *args, **kwargs):
    _SMCGRPOTrainer__init__(self, *args, **kwargs)
    generation_kwargs = getattr(self.args, "generation_kwargs", None)
    smc_cfg = generation_kwargs.get("smc", {}) if isinstance(generation_kwargs, dict) else {}
    if smc_cfg.get("return_eos") or smc_cfg.get("return_all"):
        self.enable_variable_smc()


SMCGRPOTrainer.__init__ = _smc_variable_trainer_init
#######MODIFICATION######
