...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # Mirror GRPO never uses an external reference model — KL is measured against the sampling policy.
        self.beta = args.beta
        self.ref_model = None
    ...
    def _generate_and_score_completions(
        ...
        with torch.no_grad():
            ...
            needs_old_per_token_logps = self.args.gradient_accumulation_steps % generate_every != 0
            needs_sampling_logps = self.beta != 0.0

            if needs_old_per_token_logps or needs_sampling_logps:
            ...
            old_per_token_logps = sampling_per_token_logps if needs_old_per_token_logps else None
    ...
    def _compute_loss(self, model, inputs):
        ...
        # old_per_token_logps is needed both for importance sampling (PPO clipping) and for the Mirror KL penalty.
        advantages = inputs["advantages"]
        ...
        # Compute the KL divergence between the current model and the sampling policy (Mirror GRPO)
        if self.beta != 0.0:
            sampling_per_token_logps = inputs.get("sampling_per_token_logps")
            if sampling_per_token_logps is None:
                raise ValueError(
                    "Mirror GRPO requires `sampling_per_token_logps` in the batch, but it was not provided."
                )
            per_token_kl = (
                torch.exp(sampling_per_token_logps - per_token_logps)
